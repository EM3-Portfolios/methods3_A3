...
# Pitch variability:
m_pitchVar <- lmer(scaled_sd_f0 ~ Diagnosis + (1 | ID), df) # Error: Downdated VtV is not positive definite

   #  df$scaled_sd_f0_abs <- abs(df$scaled_sd_f0)
   #  m_pitchVar <- lmer(scaled_sd_f0_abs ~ Diagnosis + (1 | ID), df)  # Model failed to converge with max|grad| = 0.093876 (tol = 0.002, component 1)Model is nearly unidentifiable: very large eigenvalue - Rescale variables?
