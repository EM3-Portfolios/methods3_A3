...
df_pitch <- readr::read_csv("pitch_data.csv")
df_pitch$X1 <- NULL
...
```{r}
# df <- readr::read_csv("SCZdf.csv")

# Removing rows with NAs
df <- na.omit(df, na.action = "omit", fill = NULL)
# Removing this extra column
df$X1 <- NULL
...
df %>% count(SANS, sort = TRUE)
df %>% count(SAPS, sort = TRUE)
df %>% count(Social, sort = TRUE)
