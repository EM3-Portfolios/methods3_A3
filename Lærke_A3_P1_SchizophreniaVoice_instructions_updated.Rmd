...
d <- read.delim("Study1D0S101T1_f0.txt")
d <- as.data.frame(d)

d_name <- "Study1D0S101T1_f0.txt"
a <- stringr::str_extract(d, "[a-z]{1,5}")
d["Language"] <- a
...
for (file in 1:length(list)) {
    ...
    d$Language <- ""
    d$Language <- str_extract(list[file], "\\d {6}")
...
df_demo <- readr::read_csv("DemographicData.csv")

# then duration data (Articulation??)
df_duration <- read.delim("Articulation.txt")
