...
#write.csv(data_lists, "pitch_data_raw.csv")
df_pitch <- readr::read_csv("pitch_data_raw.csv")
...
#write.csv(data_lists, "pitch_data.csv")
...
```{r creating the 4 models}
# We will not be using "study" as a predictor, since we are only looking at the danish studies in the first place.
pacman::p_load(lme4)


# Pitch variability:
m_pitchVar <- lm(scaled_sd_f0 ~ Diagnosis, df) # Error: Downdated VtV is not positive definite
m_pitchVar <- lmer(scaled_sd_f0 ~ Diagnosis + (1 | ID), df)
...
# The Hedge's G for speech rate is -1.26 (95%, CIs: -2.26, 0.25), and in our model we get an effect size estimate of -0.06 (95%, CIs: -0.32, 0.20). Both of these numbers are negative, indicating that the proportion of spoken time is smaller for schizophrenics than for the control group. However, in our data, the effect size is only slightly below 0, so the difference between the groups is not as big in our data.
...
# effect size: -0.02, 95% CIs: -0.21, 0.17
```
